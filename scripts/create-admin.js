...
const main = async () => {
  ...
  const specialCode = process.env.ADMIN_SPECIAL_CODE;

  if (!email || !password || !specialCode) {
    throw new Error("Missing envs: ADMIN_EMAIL, ADMIN_PASSWORD, ADMIN_SPECIAL_CODE are required");
  }

  if (!isValidFourDigitSpecialCode(specialCode)) {
    throw new Error("ADMIN_SPECIAL_CODE must be exactly 4 digits");
  }
